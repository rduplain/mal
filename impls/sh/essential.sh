...
# The R Stack - A manually managed call stack.
#
# Given the importance of the variable R as a "register" for return values,
# procedures that approximate coroutines or use recursion require a means to
# explicitly manage a call stack.
#
...
# TABLE - a data structure of generality, stored in the global namespace.
#
# Associative "Array" - store pairs with TABLE_SET, access with TABLE_GET.
# Stack - add data with TABLE_PUSH, access with TABLE_POP.
# List - add data with TABLE_PUSH, treat as an associative array with
#        one-indexed numeric keys in order to access individual items
# Set - treat as an associative array with every key having the same value.
#
...
TABLE_PUSH() {
  if [ $# -eq 2 ]; then
    eval "TABLE_PUSH_VALUE=\"\$2\""
  else
    eval "TABLE_PUSH_VALUE=\"\$R\""
  fi

  TABLE_LEN $1
  TABLE_PUSH_POSITION=$((R+1))

  TABLE_SET $1 "$TABLE_PUSH_POSITION" "$TABLE_PUSH_VALUE"

  unset TABLE_PUSH_POSITION TABLE_PUSH_VALUE
}
...
TABLE_SET() {
  ...
  if [ $# -eq 3 ]; then
    eval "TABLE_SET_VALUE=\"\$3\""
  else
    eval "TABLE_SET_VALUE=\"\$R\""
  fi

  eval "if [ -n \"\$$TABLE_SET_NAME\" ]; then TABLE_SET_EXISTED=true; fi"

  eval "$TABLE_SET_NAME=\"\$TABLE_SET_VALUE\""

  if [ -z "$TABLE_SET_EXISTED" ]; then _TABLE_INC "$1"; fi

  unset TABLE_SET_EXISTED TABLE_SET_NAME TABLE_SET_VALUE
}
